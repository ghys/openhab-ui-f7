        ...
        <f7-block v-if="currentModule">
          <f7-col>
            <f7-list inline-labels no-hairlines-md class="no-margin">
              <f7-list-input type="text" :placeholder="suggestedModuleTitle" :value="currentModule.label" required
                            @input="currentModule.label = $event.target.value" clear-button>
              </f7-list-input>
              <f7-list-input type="text" :placeholder="suggestedModuleDescription" :value="currentModule.description"
...
export default {
  ...
  computed: {
    suggestedModuleTitle () {
      if (!this.currentModuleType) return 'Name'
      const moduleType = this.currentModuleType
      const config = this.currentModule.configuration
      switch (moduleType.uid) {
        // triggers
        case 'timer.TimeOfDayTrigger':
          if (!config.time) return this.currentModuleType.label
          return 'When the time is ' + config.time
        case 'timer.GenericCronTrigger':
          if (!config.cronExpression) return this.currentModuleType.label
          return 'Cron: ' + config.cronExpression
        case 'core.ItemCommandTrigger':
          if (!config.itemName || !config.command) return this.currentModuleType.label
          return 'When ' + config.itemName + ' received command ' + config.command
        case 'core.ItemStateUpdateTrigger':
          if (!config.itemName) return this.currentModuleType.label
        ...
        case 'core.ChannelEventTrigger':
          if (!config.channelUID) return this.currentModuleType.label
        ...
        case 'media.SayAction':
          if (!config.text) return this.currentModuleType.label
          return 'Say "' + config.text + '"'
        case 'media.PlaySound':
          if (!config.sound) return this.currentModuleType.label
          return 'Say ' + config.sound
        // conditions
        case 'timer.DayOfWeekCondition':
          if (!config.days || !config.days.join || !config.days.length) return this.currentModuleType.label
          return 'If the day is ' + config.days.join(',')
        case 'core.TimeOfDayCondition':
          if (!config.startTime || !config.endTime) return this.currentModuleType.label
          return 'If the time is between ' + config.startTime + ' and ' + config.endTime
        case 'core.ItemStateCondition':
          if (!config.itemName || !config.operator || !config.state) return this.currentModuleType.label
          return 'If ' + config.itemName + ' ' + config.operator + ' ' + config.state

        default:
          return this.currentModuleType.label
      }
    },
    suggestedModuleDescription () {
      if (!this.currentModuleType) return 'Description'
      const moduleType = this.currentModuleType
      const config = this.currentModule.configuration
      switch (moduleType.uid) {
        // triggers
        ...
        default:
          return this.currentModuleType.description
      }
    }
  }
}
